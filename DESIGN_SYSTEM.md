...
В `@april/showcase` — вкладка **Mobile lab** в шапке приложения-витрины: `apps/showcase/src/MobileShowcase.tsx` (контейнеры **390×844** и **360×800**) для эталонных сценариев: нижняя панель (корень / drill-down / раскрытый поиск), `AprilModal`, формы, списки и таблицы по мере появления паттернов. Для нижней панели и поиска — §10 (фокус при открытии, `aria-label` у иконок, клавиатура для табов). Опционально — точечные **e2e** / **a11y**-прогоны на витрине для сложных блоков (как в §10 «Рост библиотеки»).
