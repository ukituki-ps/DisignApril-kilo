...
CSS-only (микросервис без React): `import '@april/tokens/css'` — переменные `--april-accent-teal-6`, нейтрали и т.д. (см. `packages/tokens/src/tokens.css`).
...
- **Опциональные peer dependencies** `@april/ui`: `react-hook-form`, `zod` — сервис подключает сам, если использует стек форм; пакет DS их не импортирует и не навязывает
